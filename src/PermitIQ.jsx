...
function Logo({ size = 32 }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{
        width: size, height: size, background: C.red,
        borderRadius: Math.round(size * 0.28),
        display: "flex", alignItems: "center", justifyContent: "center",
        color: "#fff", fontWeight: 800, fontSize: Math.round(size * 0.47),
      }}>P</div>
...
function GhostBtn({ children, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      background: "transparent", color: C.mid, border: `1px solid ${C.border}`,
      borderRadius: C.rFull, padding: "9px 18px", cursor: "pointer", transition: "all 0.15s"
    }}
...
function VerdictBadge({ verdict, label }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: C.rFull, padding: "6px 16px", marginBottom: 14
    }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 12, fontWeight: 700, color: s.fg
          }}>{c.id}</div>
    ...
    <div style={{ background: C.soft, border: `1px solid ${C.borderSoft}`, borderRadius: C.r, padding: "18px 20px", marginTop: 8 }}>
      ...
      <div style={{ display: "grid", gap: 0 }}>
        {rows.map(([key, val]) => {
          ...
          return (
            <div key={key} style={{
              display: "flex", gap: 14, fontSize: 14,
              padding: "9px 10px", margin: "0 -10px",
              borderRadius: 8,
              background: incomplete ? "#FFFDF5" : "transparent",
              borderLeft: incomplete ? "3px solid #FDE68A" : "3px solid transparent",
            }}>
...
function QuestionChips({ questions, onQueue }) {
  ...
  return (
    <div style={{ marginTop: 16 }}>
      <p style={{ fontSize: 11, color: C.sub, marginBottom: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {questions.map((q, i) => {
          ...
          return (
            <div key={i}>
              <button onClick={() => handleChipClick(i)} disabled={isAnswered} style={{
                width: "100%", textAlign: "left", cursor: isAnswered ? "default" : "pointer",
                background: isAnswered ? C.soft : C.white,
                border: `1.5px solid ${isAnswered ? C.borderSoft : isExpanded ? C.red : C.border}`,
                borderRadius: isExpanded ? "10px 10px 0 0" : 10,
                padding: "11px 15px", fontSize: 13, lineHeight: 1.5,
                fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s",
                display: "flex", alignItems: "center", gap: 9,
                color: isAnswered ? C.muted : isExpanded ? C.red : C.mid,
              }}
              ...
              onMouseLeave={e => { if (!isAnswered && !isExpanded) { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.color = C.mid; }}}>
                <span style={{ fontSize: 13, flexShrink: 0, opacity: isAnswered ? 0.5 : 1 }}>
              ...
              {isExpanded && (
                <div style={{
                  border: `1.5px solid ${C.red}`, borderTop: `1px solid ${C.borderSoft}`,
                  borderRadius: "0 0 10px 10px", background: "#FDFAF9",
                  padding: "10px 13px", display: "flex", gap: 8, alignItems: "center"
                }}>
                  <input ref={chipInputRef} value={chipInput}
                    onChange={e => setChipInput(e.target.value)}
                    onKeyDown={e => {
                      if (e.key === "Enter") { e.preventDefault(); handleQueue(i); }
                      if (e.key === "Escape") { setExpanded(null); setChipInput(""); }
                    }}
                    placeholder="Type your answer…"
                    style={{
                      flex: 1, border: "none", outline: "none", fontSize: 14,
                      color: C.black, fontFamily: "'DM Sans', sans-serif",
                      background: "transparent"
                    }} />
                  <button onClick={() => handleQueue(i)} disabled={!chipInput.trim()} style={{
                    background: chipInput.trim() ? C.red : C.borderSoft,
                    color: chipInput.trim() ? "#fff" : C.muted,
                    border: "none", borderRadius: 7, padding: "6px 13px",
                    fontSize: 12, fontWeight: 600,
                    cursor: chipInput.trim() ? "pointer" : "default",
...
function NextHint({ hint }) {
  if (!hint) return null;
  return (
    <div style={{ marginTop: 16, background: C.redLight, borderRadius: 9, padding: "11px 15px", fontSize: 13, color: C.red, fontWeight: 500 }}>
      → {hint}
    </div>
  );
}
...
function TypingDots() {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 0" }}>
      {[0,1,2].map(i => (
        <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: C.border, animation: `bloop 1.2s ${i*0.16}s ease-in-out infinite` }} />
      ))}
...
function AIAvatar() {
  return (
    <div style={{ width: 34, height: 34, borderRadius: 11, background: C.red, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 16, flexShrink: 0 }}>P</div>
  );
}
    ...
    return (
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ background: "#FFF8F6", border: `1px solid #F5D0C5`, borderRadius: "4px 16px 16px 16px", padding: "14px 18px" }}>
          <p style={{ fontSize: 14, color: C.sub, margin: "0 0 6px", lineHeight: 1.6 }}>
            Something went wrong reaching the server. Your message is saved — no need to retype.
          </p>
          {parsed.errorDetail && (
            <p style={{ fontSize: 11, color: C.muted, fontFamily: "monospace", margin: "0 0 12px", lineHeight: 1.5 }}>
              {parsed.errorDetail}
            </p>
          )}
          <button onClick={() => onRetry(retryText)} style={{
            display: "inline-flex", alignItems: "center", gap: 7,
            background: C.red, color: "#fff", border: "none",
            borderRadius: C.rFull, padding: "9px 18px",
  ...
  return (
    <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {permitFields && (
          <button onClick={() => downloadAsPDF("permit", permitFields)} style={{
...
function UserBubble({ content }) {
  return (
    <div style={{ display: "flex", justifyContent: "flex-end" }}>
      <div style={{ background: C.black, color: "#fff", borderRadius: "16px 4px 16px 16px", padding: "13px 17px", maxWidth: "76%", fontSize: 15, lineHeight: 1.65, whiteSpace: "pre-wrap" }}>
        {content}
      </div>
    </div>
  );
...
function downloadAsPDF(type, data) {
  ...
  const accentColor = "#C0461E";

  let bodyHTML = "";
  if (type === "permit") {
    const rows = Object.entries(data).filter(([, v]) => v);
    const incompleteKw = ["unknown", "verify", "n/a", "tbd", "not provided", "insufficient", "confirm"];
    bodyHTML = rows.map(([key, val]) => {
      const label = key.replace(/([A-Z])/g, " $1").replace(/^./, s => s.toUpperCase());
      const strVal = Array.isArray(val) ? val.join(" ") : String(val);
      const incomplete = incompleteKw.some(k => strVal.toLowerCase().includes(k));
      const value = Array.isArray(val)
        ? `<ul style="margin:4px 0 0 0;padding-left:18px;">${val.map(v => `<li style="margin-bottom:3px;">${v}</li>`).join("")}</ul>`
        : `<span>${val}</span>`;
      const alertBadge = incomplete ? `<span style="margin-left:8px;font-size:10px;font-weight:700;color:#92400E;background:#FFFBEB;border:1px solid #FDE68A;border-radius:4px;padding:1px 7px;vertical-align:middle;">⚠ Verify</span>` : "";
      return `<tr style="${incomplete ? "background:#FFFDF5;" : ""}"><td style="color:#717171;padding:10px 16px 10px 0;vertical-align:top;width:180px;font-size:13px;">${label}</td><td style="color:#222;font-weight:500;padding:10px 0;font-size:13px;">${value}${alertBadge}</td></tr>`;
    }).join("");
    bodyHTML = `<table style="width:100%;border-collapse:collapse;">${bodyHTML}</table>`;
  } else {
    const sevColor = { major:"#B91C1C", moderate:"#92400E", minor:"#065F46" };
    bodyHTML = data.map(c => `
      <div style="border:1px solid #ddd;border-radius:8px;padding:16px 20px;margin-bottom:12px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
          ...
          <span style="margin-left:auto;font-size:11px;font-weight:700;color:${sevColor[c.severity]||"#666"};">${c.severityLabel}</span>
        </div>
        <p style="color:#484848;font-size:13px;margin:0 0 10px;">${c.explanation}</p>
        <div style="background:#f7f7f7;border-left:3px solid ${sevColor[c.severity]||"#999"};padding:10px 14px;border-radius:0 6px 6px 0;">
          <div style="font-size:11px;font-weight:700;color:#717171;letter-spacing:0.05em;margin-bottom:4px;">REQUIRED FIX</div>
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${title}</title>
    <style>body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:800px;margin:40px auto;padding:0 24px;color:#222;}
    h1{font-size:26px;font-weight:600;color:${accentColor};border-bottom:2px solid ${accentColor};padding-bottom:14px;margin-bottom:28px;}
  ...
  try {
    ...
    // Fallback: open in new tab if Blob URL creation fails
...
function ApiKeyModal({ onSave, onClose }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: C.white, borderRadius: C.rLg, padding: "32px 36px",
        ...
        <p style={{ fontSize: 14, color: C.sub, lineHeight: 1.65, margin: "0 0 24px" }}>
          Enter your Anthropic API key to use PermitIQ outside of Claude.ai.
          Your key is stored in-session only and never transmitted anywhere except the Anthropic API.
        </p>
        <input
          ...
          type="password"
          style={{
            width: "100%", border: `1.5px solid ${C.border}`, borderRadius: C.r,
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <button onClick={() => key.trim() && onSave(key.trim())} style={{
            background: key.trim() ? C.red : C.borderSoft, color: key.trim() ? "#fff" : C.muted,
            border: "none", borderRadius: C.rFull, padding: "10px 22px",
            fontSize: 14, fontWeight: 600, cursor: key.trim() ? "pointer" : "default",
            fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s"
          }}>Save key</button>
        </div>
      </div>
...
function LandingPage({ onMode, onFreeText, onOpenApiKeyModal }) {
    ...
    <div style={{ minHeight: "100vh", background: C.white, fontFamily: "'DM Sans', sans-serif", color: C.black }}>
      <nav style={{
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <GhostBtn onClick={onOpenApiKeyModal}>Set API Key</GhostBtn>
        </div>
      </nav>

      <section style={{ padding: "80px 40px 56px", maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
        <div style={{ display: "inline-block", background: C.redLight, color: C.red, borderRadius: C.rFull, padding: "6px 18px", fontSize: 13, fontWeight: 600, marginBottom: 28, letterSpacing: "0.02em" }}>
          San Francisco Residential Permits
        </div>
        ...
        <p style={{ fontSize: 18, color: C.sub, lineHeight: 1.7, maxWidth: 560, margin: "0 auto 0" }}>
          Check feasibility, validate your plans against 70+ SF zoning rules,
          and turn permit rejections into a clear action plan.
        </p>
      </section>
      ...
      <section style={{ padding: "40px 40px 64px", maxWidth: 1080, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
          {MODES.map(mode => (
            <div key={mode.id} onClick={() => onMode(mode)} style={{
              background: C.white, border: `1.5px solid ${C.border}`, borderRadius: C.rLg,
              padding: "30px 28px 26px", cursor: "pointer", transition: "all 0.22s",
            ...
            onMouseLeave={e => { e.currentTarget.style.boxShadow = "0 1px 2px rgba(0,0,0,0.04)"; e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.borderColor = C.border; }}>
              ...
              <h3 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 26, fontWeight: 400, color: C.black, margin: "0 0 10px", lineHeight: 1.15, letterSpacing: "-0.02em" }}>
                {mode.headline}
              </h3>
              <p style={{ fontSize: 14, color: C.sub, lineHeight: 1.7, margin: "0 0 24px" }}>{mode.sub}</p>
              <div style={{ display: "flex", alignItems: "center", gap: 6, color: C.red, fontWeight: 600, fontSize: 15 }}>
                {mode.cta} <span style={{ fontSize: 17 }}>→</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: "0 40px 72px", maxWidth: 700, margin: "0 auto" }}>
        ...
        <p style={{ textAlign: "center", fontSize: 16, color: C.sub, marginBottom: 20 }}>
          Or just describe your situation — we'll figure out the right flow:
        </p>
        <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
          <textarea ref={taRef} value={freeInput}
            ...
            rows={2}
            style={{
              flex: 1, border: `1.5px solid ${C.border}`, borderRadius: C.r,
              padding: "15px 17px", fontSize: 15, color: C.black,
              fontFamily: "'DM Sans', sans-serif", resize: "none", outline: "none",
              transition: "border-color 0.2s", lineHeight: 1.55,
          ...
          <button onClick={() => freeInput.trim() && onFreeText(freeInput)}
            style={{
              background: freeInput.trim() ? C.red : C.borderSoft,
              color: freeInput.trim() ? "#fff" : C.muted,
              border: "none", borderRadius: C.r, padding: "15px 22px",
              fontSize: 15, fontWeight: 600, cursor: freeInput.trim() ? "pointer" : "default",
              fontFamily: "'DM Sans', sans-serif", transition: "all 0.18s", flexShrink: 0, alignSelf: "stretch"
            }}>Go →</button>
        </div>
      </section>
      ...
      <footer style={{ padding: "24px 40px", borderTop: `1px solid ${C.borderSoft}`, textAlign: "center" }}>
        <p style={{ fontSize: 13, color: C.muted, margin: 0 }}>
          Covers SF residential zoning (RH-1 / RH-2) · For informational use only — not legal or planning advice · Always verify with the SF Planning Department.
        </p>
      </footer>
    </div>
  );
...
function ChatView({ mode, messages, loading, onSend, onRetry, onOpenApiKeyModal, onBack }) {
  ...
  const handleQueueAnswer = (question, answer) => {
    setQueuedAnswers(prev => [...prev, { question, answer }]);
  };

  const handleRemoveQueued = (i) => {
    setQueuedAnswers(prev => prev.filter((_, idx) => idx !== i));
  };

  const handleSendAll = () => {
    if (!queuedAnswers.length || loading) return;
    const compiled = queuedAnswers
      ...
      <div style={{ borderTop: `1px solid ${C.borderSoft}`, background: C.white, padding: "16px 32px 22px" }}>
        <div style={{ maxWidth: 840, margin: "0 auto" }}>
          ...
          {queuedAnswers.length > 0 && (
            <div style={{
              background: C.soft, border: `1.5px solid ${C.border}`,
              borderRadius: C.r, padding: "14px 16px", marginBottom: 12
            }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: C.sub, letterSpacing: "0.07em", textTransform: "uppercase" }}>
                  {queuedAnswers.length} answer{queuedAnswers.length > 1 ? "s" : ""} queued
                </span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
                {queuedAnswers.map((qa, i) => (
                  <div key={i} style={{
                    background: C.white, borderRadius: 9, padding: "10px 13px",
                    border: `1px solid ${C.borderSoft}`,
                    display: "flex", alignItems: "flex-start", gap: 10
                  }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 12, color: C.muted, margin: "0 0 3px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {qa.question}
                      </p>
                      <p style={{ fontSize: 13, color: C.black, fontWeight: 500, margin: 0 }}>→ {qa.answer}</p>
                    </div>
                    <button onClick={() => handleRemoveQueued(i)} style={{
              ...
              <button onClick={handleSendAll} disabled={loading} style={{
                width: "100%", background: loading ? C.borderSoft : C.red,
                color: loading ? C.muted : "#fff", border: "none",
                borderRadius: C.rFull, padding: "12px 20px",
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
            <div style={{
              flex: 1, border: `1.5px solid ${inputFocused ? C.black : C.border}`,
              borderRadius: C.r, background: C.white, overflow: "hidden",
              ...
              <textarea ref={taRef} value={input}
                ...
                style={{
                  ...
                  background: "#fff", minHeight: 52, maxHeight: 140,
                  lineHeight: 1.55, boxSizing: "border-box",
                  boxShadow: "none", overflowY: "hidden"
                }}
              />
            </div>
            <button onClick={() => send()} disabled={!input.trim() || loading}
              style={{
                width: 52, height: 52, borderRadius: C.r, border: "none", flexShrink: 0,
                background: (input.trim() && !loading) ? C.red : C.soft,
          ...
          <p style={{ fontSize: 12, color: C.muted, textAlign: "center", marginTop: 11 }}>
          Covers SF residential zoning (RH-1/RH-2) · Not legal advice · Verify with SF Planning Dept
          </p>
        </div>
      </div>
    </div>
...
export default function PermitIQ() {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      try { parsed = JSON.parse(raw.replace(/```json|```/g, "").trim()); }
        ...
        // JSON truncated — response too long or malformed
        errorDetail = "Response was cut off. This usually means the input was too long.";
  ...
  const handleRetry = useCallback((text) => {
    // Force-reset loading in case it got stuck, then strip the failed exchange and re-send
    loadingRef.current = false;
    setLoading(false);
    ...
    <>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { margin: 0; }
        textarea { box-shadow: none; -webkit-appearance: none; background: #fff; }
      ...
      {showApiKeyModal && (
        <ApiKeyModal
          onSave={key => { apiKeyRef.current = key; setShowApiKeyModal(false); }}
          onClose={() => setShowApiKeyModal(false)}
        />
      )}
      {view === "landing"
        ? <LandingPage onMode={startMode} onFreeText={handleFreeText} onOpenApiKeyModal={() => setShowApiKeyModal(true)} />
        : <ChatView mode={activeMode} messages={messages} loading={loading} onSend={sendMessage} onRetry={handleRetry}
            onOpenApiKeyModal={() => setShowApiKeyModal(true)}
            onBack={() => { setView("landing"); setMessages([]); messagesRef.current = []; }} />
      }
    </>
  );
}
